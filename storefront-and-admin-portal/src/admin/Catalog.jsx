...
export const Catalog = () => {
  ...
  const [newUploadedFilePath, setNewUploadedFilePath] = useState(null);
  ...
  const fetchLocalUploads = async () => {
    try {
      ...
      setLocalUploads(response.data);
    } catch (err) {
      console.error("Error fetching local uploads:", err);
    }
  };
  ...
  const openEditModal = (product) => {
    ...
    const scentProfile = loadedAccords.length > 0 ? loadedAccords[0].name : product.scent_profile;
    setFormData({
      name: product.name,
      brand: product.brand,
  ...
  const uploadFileDirectly = async (file) => {
    ...
    try {
      setUploading(true);
      // Try native multipart form data upload
      ...
      // Fallback: Read as base64 and upload via standard JSON payload
      try {
  ...
  >
    ...
    /* Catalog Table Display */
      ...
      {loading ? <div className="flex flex-col items-center justify-center h-[40vh] gap-4">
        ...
        </div> : <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden shadow-xl">
    ...
    /> : <Layers className="w-5 h-5 text-brand-gold/40" />}
                        ...
                        <td className="py-4 px-6">
                          <div className="flex flex-wrap gap-1.5 max-w-[220px]">
                            {product.volumes.length === 0 ? <span className="text-xs text-brand-cream/35 italic">No price tags set</span> : product.volumes.map((vol) => <span
      key={vol.id}
      className={`inline-flex px-1.5 py-0.5 rounded-sm text-[10px] font-semibold border ${vol.is_available ? "text-brand-gold border-brand-gold/20 bg-brand-gold/5" : "text-brand-cream/40 border-white/[0.04] bg-white/[0.01] line-through"}`}
    >
                                  {vol.size}: ₱{vol.price}
                                </span>)}
                          </div>
      ...
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider border cursor-pointer transition ${product.is_active ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20 hover:bg-emerald-500/20" : "text-brand-cream/40 bg-white/[0.02] border-white/[0.08] hover:bg-white/[0.06]"}`}
    >
                            {product.is_active ? <>
    ...
    /* Pagination Controls */
          ...
          {pagination && pagination.last_page > 1 && <div className="bg-white/[0.02] border-t border-white/[0.06] px-6 py-4 flex items-center justify-between text-xs text-brand-cream/60">
              <div>
  ...
  >
    ...
    /* Create/Edit Product Glass Sheet Modal */
      ...
      <AnimatePresence>
        {isProductModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
  ...
  >
                    ...
                    {formData.accords.length > 0 && <p className="text-[10px] text-brand-gold/60 mt-1 italic">
  ...
  >
    ...
    /* Main Accords & Progression Tiers */
                  ...
                  <div className="p-5 bg-white/[0.02] border border-white/[0.06] rounded-xl space-y-4 md:col-span-2">
                    ...
                    {formData.accords.length === 0 ? <div className="py-6 text-center text-brand-cream/35 italic text-xs bg-black/20 border border-dashed border-brand-gold/15 rounded-sm">
                        No main accords configured. The storefront will only display the primary Scent Profile.
                      </div> : <div className="space-y-3">
                        {formData.accords.map((accord, idx) => <div
    key={idx}
  ...
  >
                    ...
                    {formData.accords.length < 5 && <button
    type="button"
  ...
  /> : <div className="text-[10px] text-brand-cream/30 text-center p-2 uppercase tracking-wider">No Image</div>}
                        {uploading && <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
  ...
  >
    ...
    /* Pricing / Volume Management Glass Modal */
      ...
      <AnimatePresence>
        {isPricingModalOpen && pricingProduct && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
    initial={{ opacity: 0, scale: 0.95 }}
    animate={{ opacity: 1, scale: 1 }}
    exit={{ opacity: 0, scale: 0.95 }}
  ...
  >
    ...
    /* Active list of sizes */
              ...
              <div className="space-y-4 mb-8">
                ...
                {pricingProduct.volumes.length === 0 ? <div className="py-8 text-center text-brand-cream/30 italic text-sm bg-white/[0.01] border border-white/[0.04] rounded-sm">
                    No size/pricing variations set for this product. Use the form below to append sizes.
                  </div> : <div className="space-y-3">
                    {pricingProduct.volumes.map((vol) => <div
    key={vol.id}
