...
export default function ProductModal({ product, onClose, selectedVolume }) {
  ...
  const currentPrice = hasVolumes
    ? (product.volumes.find(v => v.size === selectedVolume)?.price || product.volumes[0].price)
    : 0;
        ...
        <div className="md:w-7/12 p-6 md:p-10 flex flex-col gap-8">
          ...
          <div className="mt-auto pt-6 border-t border-brand-gold/20 flex items-center justify-between">
            ...
            <button
              ...
              disabled={!hasVolumes}
              className={`flex items-center gap-2 px-8 py-3 font-bold uppercase tracking-widest text-sm rounded-full transition-all ${
                hasVolumes
                  ? 'bg-brand-gold text-brand-emerald-dark hover:bg-brand-gold-light shadow-[0_0_15px_rgba(212,175,55,0.2)] hover:shadow-[0_0_25px_rgba(212,175,55,0.4)]'
                  : 'bg-brand-emerald-dark/50 text-brand-cream/30 border border-brand-gold/10 cursor-not-allowed'
              }`}
            >
              {hasVolumes ? 'Add to Inquiry Bag' : 'Unavailable'} <ArrowRight size={16} />
            </button>
          </div>
